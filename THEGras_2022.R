...
setwd("C:\\users\\lio\\Documents\\Research\\GrassCast\\R_access_script")  
library(ggplot2)
library("writexl")
library(ggthemes)
library(dplyr)
...
#Subsest by year
GC20 <- subset(GC, year == '2020', select=c("gridID","spring_delta_anpp"))
GC20 <- subset(GC, year == '2020', select=c("spring_delta_ndvi","lon","lat"))

write_xlsx(GC20, 'GC202.csv')
GC20_1 <- aggregate(GC20,GC20$Point, FUN = average)
...
NDVIAN <- ggplot(GCY, aes(x = year, y = spring_delta_ndvi, fill=spring_delta_anpp)) +
  geom_line(position = "dodge", colour = "black", size = 0.25)
...
Factor = "spring_delta_ndvi"

GC20 <- subset(GC, year == '2020', select=c("spring_delta_ndvi","lon","lat"))
#Select the quatity of groups in factor 

GC20_2<- ntile("GC$spring_delta_ndvi",4)
